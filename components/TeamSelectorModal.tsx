...
const TeamSelectorModal: React.FC<TeamSelectorModalProps> = ({ isOpen, onClose, onSelectTeam, currentTeam }) => {
    ...
    useEffect(() => {
        ...
        const isSaved = savedTeams.some(t => t.name === currentTeam);
                             ...
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {savedTeams.map(team => (
                                    <button
                                        ...
                                        className={`p-3 rounded-xl transition-all duration-200 border text-left flex items-center gap-3 relative overflow-hidden ${
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                            {TEAMS_CONFIG.map(team => (
                                <button
                                    ...
                                    className={`p-4 rounded-xl transition-all duration-200 transform hover:scale-105 shadow-md flex flex-col items-center justify-center text-center gap-2 h-24 leading-tight ${
                                        currentTeam === team.name 
