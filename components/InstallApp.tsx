...
const InstallApp: React.FC<InstallAppProps> = ({ className, variant = 'button' }) => {
        ...
        return (
            <>
                <button
                    ...
                    className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl bg-slate-800 border border-slate-700 hover:border-cyan-500/50 hover:bg-slate-750 transition-all group ${className}`}
                    title="Instalar App"
                >
                    <div className="p-3 bg-slate-900 rounded-full group-hover:bg-slate-800 transition-colors">
                        <DownloadIcon className="h-6 w-6 text-slate-400 group-hover:text-cyan-400" />
                    </div>
                    <span className="text-sm font-bold text-slate-300 group-hover:text-white">Instalar App</span>
                </button>
    ...
    return (
        <>
            <button
                onClick={handleInstallClick}
                className={className || "flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800/80 hover:bg-slate-700 border border-slate-600 hover:border-cyan-500/50 text-white font-semibold transition-all shadow-lg backdrop-blur-sm group"}
                title="Instalar App"
            >
...
const IOSInstructions: React.FC<{ onClose: () => void }> = ({ onClose }) => (
    <div className="fixed inset-0 bg-black bg-opacity-90 z-[60] flex flex-col justify-end pb-8 animate-fade-in" onClick={onClose}>
