...
- **Limpieza de Código Muerto Realizada:**
  ...
  - **Saneamiento de Base de Datos en Supabase:** Migración ejecutada con éxito (`deprecate_shot_chart_and_cleanup.sql`). Se completó el backfill histórico de tiros a `tally_stats`, la normalización de `game_mode: 'stats-tally'`, y la eliminación definitiva de las tablas `shots`, `fixture` y `tournaments`. La capa de cliente (`SyncContext.tsx`, `useAdminGames.ts`, `dbAdapters.ts`) opera exclusivamente sobre `games` y `tally_stats` con cero consultas residuales a tablas deprecadas.
