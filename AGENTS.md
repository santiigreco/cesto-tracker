...
- **Limpieza de Código Muerto Realizada:**
  ...
  - **Federación Correntina y Rediseño de Home:** Se integraron los 8 clubes oficiales de la Federación Correntina de Cestoball (`Jaguareté`, `Hércules`, `Quilmes`, `Regatas`, `San Martín (Ctes)`, `Banco Provincia`, `Córdoba`, `Colón`) con escudos vectoriales en `TeamLogo.tsx`, selector categorizado por pestañas (`FeMeCe` vs `Corrientes`) y buscador en tiempo real con soporte de clubes libres en `TeamSelectorModal.tsx`. Rediseño total de la Home como Sports Command Center con live preview de planilla técnica y métricas comunitarias.
