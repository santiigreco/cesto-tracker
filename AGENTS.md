...
- **Limpieza de Código Muerto Realizada:**
  ...
  - **Federaciones Capital y Corrientes, y Rediseño de Home:** Se integraron los 8 clubes oficiales de Corrientes (`Jaguareté`, `Hércules`, `Quilmes`, `Regatas`, `San Martín (Ctes)`, `Banco Provincia`, `Córdoba`, `Colón`) junto a los clubes de Capital, con escudos vectoriales en `TeamLogo.tsx`, selector categorizado por pestañas (`Capital` vs `Corrientes`) y buscador en tiempo real con soporte de clubes libres en `TeamSelectorModal.tsx`. Rediseño total de la Home como Sports Command Center con banner de bienvenida a la Federación de Corrientes, live preview de planilla técnica y métricas comunitarias.
  - **Persistencia Confiable en Producción y Auditoría (`match_activity_logs`):** Resuelta la causa raíz de la fuga de datos por rechazo 400 (`tournament_id` en payload de `games`). Se eliminó el silenciamiento de errores en autoguardado de `SyncContext.tsx`, se habilitó telemetría con prefijos `[PERSISTENCE_SUCCESS]` y `[PERSISTENCE_ERROR]`, se creó la tabla de auditoría `match_activity_logs` con RLS e índices B-tree, se mejoró la detección de cambios mediante firma compuesta y se incorporó listener de ciclo de vida (`beforeunload`) para forzar la persistencia antes de cerrar o cambiar de pestaña.
  - **Rediseño Minimalista de Landing Page (Completado):** Eliminación de sobrecarga visual y estilo de plantilla genérica (contadores acumulados estáticos, marcadores simulados confusos y cards flotantes). Implementación de flujo de conversión directo con CTA unificado en Header ('Anotar Partido'), Hero deportivo con titular conciso y botón principal centrado ('Crear Nueva Planilla'), Visual Showcase minimalista en HTML/CSS de la interfaz de planilla técnica en vivo, y Bento Grid de 3 pilares deportivos (Anotación Ágil, Precisión Técnica, Estadísticas Consolidadas).
  - **Optimización Mobile UX, Resiliencia ante F5 y Regla de 6 Faltas (Completado):** Auditoría destructiva de usabilidad táctil completada. Hidratación síncrona diferida en `GameContext` evitando expulsión involuntaria a `/setup` tras recargar la app; mutex antirrebote en `SyncContext` contra duplicación en Supabase por rage-clicks; áreas táctiles elevadas a `>= 44px` en botones de anotación y tanteador; prevención de auto-zoom en iOS Safari (font-size 16px en inputs); columna fija sticky para tabla de rendimiento; botones de header conectados en `MatchRoute`; regla reglamentaria oficial de 6 faltas personales con alertas visuales de "En capilla (5F)" y "Cambio obligatorio (6F)".
